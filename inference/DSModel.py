...
class Linear(nn.Module):
    ...
    def __init__(self,in_features:int,out_features:int,bias:bool=False,dtype=None):
        ...
        self.weight=nn.Parameter(torch.empty(out_features,in_features,dtype=dtype or Linear.dtype)) # 初始化权重，特征数为out_features*in_features 矩阵乘法
        if self.weight.element_size()==1: # 如果权重的元素大小为1
            scale_out_features=(out_features+block_size-1)//block_size # 计算输出特征数
            scale_in_features=(in_features+block_size-1)//block_size # 计算输入特征数
            self.weight.scale=self.scale=nn.Parameter(torch.empty(scale_out_features,scale_in_features,dtype=torch.float32)) # 初始化缩放因子

        else:
            self.register_parameter("scale",None)  # pytorch中的register_parameter方法用于注册模型参数，之后被pytorch自动追踪，用于参数更新（训练），模型保存加载和设备迁移
        if bias:
...
class ColumnParallelLinear(Linear):
    ...
    """ 
    def __init__(self,in_features:int,out_features:int,bias:bool=False,dtype=None):
        assert out_features%world_size==0,f"out_features {out_features} must be divisible by world_size {world_size}" # 保证输出特征数是world_size的整数倍
        self.part_out_features=out_features//world_size # 每个设备上的输出特征数
        super().__init__(in_features,self.part_out_features,bias,dtype) # 调用父类的初始化函数
...
class RowParallelLinear(Linear):
    ...
    def __init__(self,in_features:int,out_features:int,bias:bool=False,dtype=None):
        assert in_features%world_size==0,f"input features must be divisible by world_size {world_size}"
...
class MLA(nn.Module):
    ...
    def __init__(self,args:ModelArgs):
        ...
        self.n_local_heads=args.n_heads//world_size  # 每个设备上的注意力头数(用于分布式训练)
        ...
        if self.q_lora_rank ==0:  # 如果不使用LoRA
            ...
            self.wq_b=ColumnParallelLinear(self.q_lora_rank,self.n_heads*self.qk_head_dim)  # 第二个变换

        # Key-Value变换矩阵的初始化
        self.wkv_a=Linear(self.dim,self.kv_lora_rank+self.qk_rope_head_dim)  # KV的第一个变换
        self.kv_norm=RMSNorm(self.kv_lora_rank)  # KV的归一化层
        self.wkv_b=ColumnParallelLinear(self.kv_lora_rank,self.n_heads*(self.qk_nope_head_dim+self.v_head_dim))  # KV的第二个变换
        
        # 输出投影矩阵
        self.wo=RowParallelLinear(self.n_heads*self.v_head_dim,self.dim)  # 多头注意力输出的合并变换
        ...
        if args.max_seq_len>args.original_seq_len:
            ...
            self.softmax_scale=self.softmax_sclae*mscale*mscale  # 根据序列长度调整缩放因子
        ...
        if attn_impl=='naive':  # 朴素实现方式
            ...
            self.register_buffer('pe_cache',torch.zeros(args.max_batch_size,args.max_seq_len,self.qk_rope_head_rank),persistent=False)
